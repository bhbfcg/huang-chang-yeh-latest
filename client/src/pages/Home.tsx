// COCO editorial system: quiet ivory surfaces, oversized geometric type, and a tactile before/after reveal.
...
function HeroCompare() {
  ...
  return <section className="hero-compare" ref={stageRef} onPointerEnter={(event) => { setIsTouch(event.pointerType !== "mouse"); updatePoint(event.clientX, event.clientY); }} onPointerMove={(event) => { if (event.pointerType !== "mouse" && !event.buttons) return; updatePoint(event.clientX, event.clientY); }} style={{ "--cursor-x": `${point.x}%`, "--cursor-y": `${point.y}%`, "--mobile-reveal": `${slider}%` } as React.CSSProperties} aria-label="Before and after interior transformation">
    <img className="hero-image hero-before" src={assets.heroBefore} alt="Renovation before view of a bright interior" width={1920} height={1080} loading="eager" decoding="async" />
    <img className={`hero-image hero-after ${isTouch ? "hero-after-touch" : ""}`} src={assets.heroAfter} alt="Renovation after view of a bright interior" width={1920} height={1080} loading="eager" fetchPriority="high" decoding="async" />
    ...
    <div className="mobile-compare-control"><label htmlFor="compare-slider">Two scales</label><input id="compare-slider" type="range" min="0" max="100" value={slider} aria-label="Reveal the after image" onChange={(event) => { setIsTouch(true); setSlider(Number(event.target.value)); }} /></div>
  </section>;
}

const projects = [
  { no: "01", type: "Cultural space", title: "茶藝館", meta: "Cultural space / Tea house", image: assets.teahouse, slug: "teahouse-graduation-project" },
  { no: "02", type: "Architecture concept", title: "土地公廟", meta: "Sacred space / Concept study", image: assets.temple, slug: "light-church-tudigong-temple" },
  { no: "03", type: "Residential visualization", title: "璞真永吉 13 樓", meta: "Residential visualization / Interior study", image: "/assets/picture1-yongji13-01.webp", slug: "yongji-13f-visualization" },
];
